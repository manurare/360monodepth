...
def run_depth(img_batch, model_path, Net, utils, target_w=None, output_path='depth_subviews/'):
    ...
    if not os.path.exists(model_path):
        raise RuntimeError("the MiDaS model {} doesn't exist.".format(model_path))
    ...
    for ind in range(0, img_batch.shape[0]):

        print("processing({}/{})".format(ind + 1, img_batch.shape[0]))

        # input
        img = cv2.cvtColor(img_batch[ind], cv2.COLOR_BGR2RGB) / 255.
        w = img.shape[1]
        ...
        # filename = os.path.join(output_path, '{:04}'.format(ind))
        # np.save(filename + '.npy', depth)
        # utils.write_depth(filename, depth, bits=2)

    print("finished")
    return np.array(depth_imgs)
